#!/usr/bin/env python3
"""
基金组合回测工具 - 支持场内 ETF 腾讯证券数据 + 场外基金天天基金数据
"""
...
# 场内 ETF 列表（使用腾讯证券 API）
ETF_CODES = {'159985', '518880', '159934', '513050', '513100', '513500', '510300'}


class HybridFundBacktester:
    ...
    def fetch_etf_data(self, fund_code):
        """使用新浪财经 API 获取场内 ETF 历史行情（收盘价，前复权）"""
        cache_file = self.cache_dir / f"{fund_code}_etf.csv"
        ...
        print("  📡 获取 ETF 行情...", end=" ", flush=True)
        
        # 新浪财经历史数据 API（前复权）
        url = "http://money.finance.sina.com.cn/quotes_service/api/json_v2.php/CN_MarketData.getKLineData?symbol=sz{}&scale=240&ma=no&datalen=10000".format(fund_code)
        ...
        try:
            ...
            df['单位净值'] = df['close'].astype(float)
            df = df[['date', '单位净值']].copy()
    ...
    def fetch_fund_nav(self, fund_code):
        """使用天天基金 pingzhongdata JS 接口获取场外基金历史净值"""
        ...
        print("  📡 获取净值...", end=" ", flush=True)
        
        url = "http://fund.eastmoney.com/pingzhongdata/{}.js".format(fund_code)
        headers = ['-H', 'Referer: http://fund.eastmoney.com/', '-H', 'User-Agent: Mozilla/5.0']
        ...
        try:
            import re
            match = re.search(r'Data_netWorthTrend\s*=\s*(\[.*?\]);', result.stdout, re.DOTALL)
            if not match:
                print("❌ 未找到数据")
                return pd.DataFrame()
    ...
    def load_all_funds_data(self):
        ...
        for code in self.fund_codes:
            ...
            col_name = '净值日期' if '净值日期' in df.columns else 'date'
            df = df.set_index(col_name)['单位净值']
            df = df.rename(code)
            all_nav[code] = df
    ...
    def report(self, m):
        lines = [
            "=" * 70,
            "📊 基金组合回测（场内 ETF 用腾讯证券 + 场外基金用天天基金）",
            "=" * 70, "",
            "📋 配置:"
        ]
        for c, w in zip(m['fund_codes'], m['weights']):
            etf_mark = " [ETF]" if c in ETF_CODES else ""
            lines.append("   {} {:.1f}%{}".format(c, w*100, etf_mark))
        lines.extend([
...
def main():
    parser = argparse.ArgumentParser(description='基金组合回测（支持场内 ETF）')
    parser.add_argument('--funds', required=True, help='基金代码列表，逗号分隔')
    parser.add_argument('--weights', required=True, help='配置比例，逗号分隔')
    parser.add_argument('--start-date', default='2019-01-01', help='开始日期 YYYY-MM-DD')
    parser.add_argument('--end-date', default=None, help='结束日期 YYYY-MM-DD')
    parser.add_argument('--rebalance-month', type=int, default=1, help='再平衡月份 (1-12)')
    parser.add_argument('--initial-capital', type=int, default=1000000, help='初始资金')
    parser.add_argument('--json', action='store_true', help='输出 JSON 格式')
    parser.add_argument('--cache-dir', default='~/.openclaw/workspace/skills/fund-optimizer/cache', help='缓存目录')
    
    args = parser.parse_args()
    
    funds = args.funds.split(',')
    weights = [float(w) for w in args.weights.split(',')]
    
    if abs(sum(weights) - 1.0) > 0.001:
        print("⚠️  权重总和不为 1，正在归一化...")
        total = sum(weights)
        weights = [w/total for w in weights]
    
    backtester = HybridFundBacktester(
        fund_codes=funds,
        weights=weights,
    ...
    if args.json:
        ...
        print(backtester.report(metrics))
